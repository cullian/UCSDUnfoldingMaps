 ...
 * team
 * 
 * @author William Cullian Date: April 4, 2017
 */
public class EarthquakeCityMap extends PApplet {
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker country : countryMarkers) {
			if (isInCountry(earthquake, country)) {
				// lets keep count
				if (country.getProperties().containsKey("EQCOUNT")) {
					country.setProperty("EQCOUNT", country.getIntegerProperty("EQCOUNT") + 1);
				} else {
					country.setProperty("EQCOUNT", 1);
				}
				return true;
			}
		}
	...
	private void printQuakes() {
		...
		for (Marker country : countryMarkers) {
			// if there was an earthquake in this country
			if (country.getProperties().containsKey("EQCOUNT")) {
				System.out.println(country.getStringProperty("name") + " : " + country.getProperty("EQCOUNT"));
			}
		}
		// // count sea quakes
		...
		// }
		// }
		System.out.println("Earthquakes in oceans : " + oceanQuakeCount);
	}
